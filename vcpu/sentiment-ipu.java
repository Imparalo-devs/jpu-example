...
public class SentimentIPU {
    ...
    public static String Input_Security_Filter_Sanitization__iLU(String variableToCheck, int maxSize) {
        ...
        if (variableToCheck.matches(".*(?:<script|eval|function|constructor|prototype|__proto__).*")) {
            return "";
        }

        if (variableToCheck.matches(".*(?:base64|hex|decode|encode|eval|system|exec|passthru|shell_exec|exec|popen|proc_open).*")) {
            return "";
        }

        if (variableToCheck.matches(".*(?:you are now|system:|developer:|role:|override).*")) {
            return "";
        }
    ...
    public static Map<String, Object> runSentimentiPU(Object chatInput) {
        ...
        try {
            ...
            CompletableFuture<String> sanitizedFuture = CompletableFuture.supplyAsync(() -> Input_Security_Filter_Sanitization__iLU(SentimentIPU.chatInput, 1000));

            CompletableFuture<Void> combinedFuture = CompletableFuture.allOf(sentimentFuture, sanitizedFuture);
            combinedFuture.get(10, TimeUnit.SECONDS);

            Map<String, Object> sentimentResult = sentimentFuture.get();
            String sanitizedResult = sanitizedFuture.get();
            ...
            long endTime = System.currentTimeMillis();
            System.out.println("ERR - " + e.getMessage() + " - " + (endTime - startTime) + "ms");
            return null;
        }
    }
