...
public class SentimentIPU {
    ...
    public static String Input_Security_Filter_Sanitization__iLU(String variableToCheck, int maxSize) {
        ...
        variableToCheck = StringEscapeUtils.escapeHtml4(variableToCheck);
        variableToCheck = Jsoup.clean(variableToCheck, "", Whitelist.none(), new Document.OutputSettings().prettyPrint(false));
        if (variableToCheck.matches(".*(?:UNION|SELECT|INSERT|UPDATE|DELETE|DROP|CREATE|ALTER|TRUNCATE|EXECUTE|EXEC|DECLARE|CREATE|ALTER|TRUNCATE|DROP).*")) {
            return "";
        }
        if (variableToCheck.matches(".*(?:<script|eval|function|constructor|prototype|__proto__).*")) {
            return "";
        }
        if (variableToCheck.matches(".*(?:base64|hex).*")) {
            return "";
        }
    ...
    public static void runSentimentiPU(String chatInput) {
        try {
            ...
            CompletableFuture<Map<String, Object>> sentimentFuture = Sentiment_Analyzer_iLU(chatInput);
            String sanitizedInput = Input_Security_Filter_Sanitization__iLU(chatInput, 1000);
            Map<String, Object> sentimentResult = sentimentFuture.get(10, TimeUnit.SECONDS);
